...
if __name__ == "__main__":
    ...
    WINDOW_SIZE: int = 800
    TABLE_UPDATE_FREQ: int = 20

    # 3. Initializes pygame and rich instances.
    pygame.init()
    ...
    config_simulation_visualizer = ConfigSimulationVisualizer(
        simulation_size = SIMULATION_SIZE,
        window_size = WINDOW_SIZE
    )
    simulation_visualizer = SimulationVisualizer(
        config_simulation_visualizer = config_simulation_visualizer,
        resource_manager = resource_manager,
    ...
    data_visualizer = DataVisualizer(
        config_data_visualizer = config_data_visualizer,
        matplotlib_auto_plotter = matplotlib_auto_plotter,
        agent_manager = agent_manager
    )
    ...
    # 6. Defines starting values for the loop.
    running: bool = True

    # 7. Main Loop.
    while running:
        # A. Checks, whether the simulation is still running.
        running = simulation_visualizer.simulation_loop()

        # B. Loops the resources.
        resource_manager.resource_loop()

        # C. Loops the agents.
        agent_manager.agent_loop(map_temperature=25)

        # D. Updates the stats history.
        data_visualizer.update_stats_history()

    # 8. Generates the plots for the history trackers.
